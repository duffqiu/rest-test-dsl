...
import org.duffqiu.rest.test.actor.RestTaskMessage
import org.duffqiu.rest.test.actor.RestTaskMessage
    ...
    describe("Rest server and client DSL Testing") {
        ...
        it("Should be OK using rest style with request created by generator with mutiple works") {
                ...
                i =>
                    val worker = new RestClientWorkActor("Client" + i, masterActor, aServer, LOCAL_HOST, aServer.serverPort, {
                        (server, resource, req, operation, resp, resultResponse) =>
                            ...
                            (server, resource, req, operation, resp, resultResponse) match {
                                ...
                                case _ => println("[Test]got unknow message: " + req)
                            }
                    })
                    masterActor ! worker
            }
            ...
            req_resp.par.foreach {
                t =>
                    masterActor ! RestTaskMessage(resource1, t._1, DELETE, t._2, CLIENTERROR)
            }

            req_resp2.par.foreach {
                t =>
                    masterActor ! RestTaskMessage(resource1, t._1, QUERY, t._2, SUCCESS)
            }
